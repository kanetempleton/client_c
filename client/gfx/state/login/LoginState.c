...
void initLoginState(LoginState* state, SDL_Renderer* renderer) {
    TTF_Font* font = TTF_OpenFont("data/assets/fonts/Arial.ttf",16);
...
void renderLoginState(LoginState* state, TTF_Font* font, SDL_Color color, int textUpdateRequired) {
    ...
    SDL_Surface* Loading_Surf = IMG_Load("data/assets/background.png"); //TODO: init the background in init method so we dont have to do it over and over again
            ...
            switch (i) {
                case 0: // click "enter" button
                    if ((strlen(state->usernameText->build)>0) && (strlen(state->passwordText->build)>0)) {
                        sendLoginRequest(yourPlayer,state->usernameText->build,state->passwordText->build);
